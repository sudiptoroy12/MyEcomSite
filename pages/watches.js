...
const Watches = ({products}) => {
  ...
  <div className="container px-5 py-10 ">
    ...
    {Object.keys(products).length === 0 && <p>No watches available</p> }
...
export async function getServerSideProps(context) {
 ...
 for(let item of products)
   if(item.title in watches){
     if(!watches[item.title].color.includes(item.color) && item.avaiableQty > 0){
       watches[item.title].color.push(item.color)
     }
     if(!watches[item.title].size.includes(item.size) && item.avaiableQty > 0){

...
const Jackets = ({products}) => {
  ...
  <div className="container px-5 py-10 ">
    ...
    {Object.keys(products).length === 0 && <p>No jackets available</p> }
...
export async function getServerSideProps(context) {
 ...
 for(let item of products)
   if(item.title in jackets){
     if(!jackets[item.title].color.includes(item.color) && item.avaiableQty > 0){
       jackets[item.title].color.push(item.color)
     }
     if(!jackets[item.title].size.includes(item.size) && item.avaiableQty > 0){
...
return {
props: {products:JSON.parse(JSON.stringify(jackets)) },
}
}
